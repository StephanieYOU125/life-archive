...
function visibleRows(){
  ...
  const rows=timeline.filter(x=>(filter==='全部'||x.status===filter)&&(!q||[x.category,x.time,x.identity,x.resume,x.result,x.status,x.note].join(' ').toLowerCase().includes(q)));
  return sortTimelineRows(rows);
}

function render(){
  const body=document.getElementById('tlBody');if(!body)return;const rows=visibleRows();
  body.innerHTML=rows.length?rows.map((x,index)=>`<tr data-id="${x.id}">
...
function bind(){
  ...
  document.getElementById('tlAdd')?.addEventListener('click',()=>{timeline.unshift({id:uid(),category:'其他',time:'',identity:'',resume:'',result:'',status:'待確認',note:''});save();filter='全部';search='';if(searchEl)searchEl.value='';if(filterEl)filterEl.value='全部';render();document.querySelector('#tlBody tr:first-child [data-f="time"]')?.focus();});

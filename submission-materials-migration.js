(()=>{
  ...
  const RELOAD_KEY='submissionMaterials20260817Reloaded';
  ...
  for(const [title,question] of chapterPlan){
    ...
    const matchIndex=existingChapters.findIndex((c,i)=>!used.has(i)&&key.length>5&&normalized(c.title).includes(key));
    if(matchIndex>=0){
      used.add(matchIndex);
      const old=existingChapters[matchIndex];
      curated.push({...old,title,question:old.question||question,submissionPlan:true});
    }else curated.push({id:uid(),title,question,draft:'',submissionPlan:true});
  }
  ...
  state.migration[MIGRATION_KEY]={appliedAt:new Date().toISOString(),chapterPlan:20,materialsAdded:added,source:'投稿準備.docx'};

...
function normalizeMaterial(item){return {...item,time:item.time||'',timePrecision:item.timePrecision||'待確認',tags:item.tags||'',chapterId:item.chapterId||'',timelineId:item.timelineId||'',stage:item.stage||'靈感箱',evidence:item.evidence||'',feelings:item.feelings||'',reflection:item.reflection||''}}
...
function filteredMaterials(){
  ...
  if(searchTerm.trim()){
    ...
    rows=rows.filter(x=>[x.title,x.content,x.evidence,x.feelings,x.reflection,x.time,x.tags,chapterTitle(x),timelineTitle(x),x.stage].join(' ').toLowerCase().includes(q));
  }
...
function toolbarHtml(){
  return `<div class="materials-toolbar">
    <input id="materialSearch" class="materials-search" value="${esc(searchTerm)}" placeholder="搜尋素材、證據、感受、理解、標籤、時間、篇章……">
...
function editorFields(item,always=false){
  return `<div class="material-editor-fields${always?' always':''}">
    ...
    <label class="material-field"><span>標籤</span><input data-material-field="tags" value="${esc(item.tags||'')}" placeholder="例：東京大學, 人脈, 問對人, 決策"></label>
...
function addStyles(){
  ...
  @media(max-width:1050px){.material-compact-main{grid-template-columns:90px minmax(220px,2fr) minmax(120px,1fr) 80px 24px}.material-chapter{display:none}.material-library.cards{grid-template-columns:repeat(2,minmax(0,1fr))}.material-story-grid{grid-template-columns:1fr}}
